...
public:
	...
	friend Color operator+(Color lhs, const const Color& rhs)
	{
...
private:
	uint8_t getA(Colors col)
	{
		uint8_t r = ((static_cast<int>(col) >> 24) & 0xFF);
		return r;
	}
	uint8_t getR(Colors col)
	{
		uint8_t r = ((static_cast<int>(col) >> 16) & 0xFF);
		return r;
	}

	uint8_t getG(Colors col)
	{
		uint8_t r = ((static_cast<int>(col) >> 8) & 0xFF);
		return r;
	}

	uint8_t getB(Colors col)
	{
		uint8_t r = ((static_cast<int>(col)) & 0xFF);
		return r;
	}
	/*
	void limit()
